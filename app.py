...
if st.button(
    ...
    if username == "" or password == "":
        st.error(
            "Логин мен құпия сөзді толық енгізіңіз."
        )
...
st.markdown(
    '<div class="kasym-subtitle">👑 PRESIDENT PANEL</div>',
    unsafe_allow_html=True
)

st.success(
    f"Қош келдіңіз, {st.session_state.full_name}!"
)

st.info(
    "Президенттің негізгі міндеті — жүйедегі аккаунттарды басқару."
)
...
st.write(
    "👑 **Президент** — аккаунттарды басқарады."
)

st.write(
    "👨‍💼 **Премьер министр** — сұрақтар базасын басқарады."
)

st.write(
    "👤 **Оқушы** — тест тапсырады және нәтижесін көреді."
)
```
...
if st.button(
    ...
    if (
        ...
        st.success(
            f"✅ {name} үшін аккаунт жасалды."
        )

        st.info(
            f"Логин: {new_username}"
        )
...
for index, user in enumerate(users):
    ...
    # Президент аккаунтын өшіруге болмайды
    if user.get("role") != "president":

        if st.button(
            f"🗑️ Өшіру: {user.get('username')}",
            key=f"delete_user_{index}",
            use_container_width=True
        ):
...
st.markdown(
    '<div class="kasym-subtitle">👨‍💼 ПРЕМЬЕР МИНИСТР</div>',
    unsafe_allow_html=True
)
...
if st.session_state.role != "prime_minister":
    st.error(
        "Бұл бөлімге тек Премьер министр кіре алады."
    )
    return
...
if st.button(
    ...
    if (
        ...
        questions[selected_subject].append(
            new_q
        )
...
subject_q = questions.get(
    selected_subject,
    []
)

st.write(
    f"Жалпы сұрақ саны: **{len(subject_q)}**"
)
...
for idx, q in enumerate(subject_q):
    ...
    with st.expander(
        ...
        if st.session_state.role == "prime_minister":
            ...
            if st.button(
                f"🗑️ Өшіру №{idx + 1}",
                key=f"del_{selected_subject}_{idx}"
            ):
...
if not my_results:
    ...
    st.info(
        "Әзірге тапсырылған тест нәтижесі жоқ."
    )
...
for number, item in enumerate(
    ...
    st.markdown(
        ...
        <div class="card">
            <h3>
                📝 {number}-тест —
                {item.get("subject", "Пән")}
            </h3>

            <p>
                Нәтиже:
                <b>{item.get("percent", 0)}%</b>
            </p>
...
if not my_results:
    ...
    st.info(
        "Прогресс көрсету үшін "
        "алдымен кемінде бір тест тапсырыңыз."
    )
...
for subject, scores in subject_stats.items():
    ...
    st.write(
        f"**Үздік нәтиже:** {best_score}%"
    )

    st.write(
        f"**Тест саны:** {test_count}"
    )
