    ...
    for subject in all_subjects:
        ...
        st.markdown(
            ...
            <div class="subject-card">
                <h3>{subject}</h3>
                <p class="small-text">Сұрақ саны: {count}</p>
            </div>
            """,
            unsafe_allow_html=True
        )
...
def add_question_page():
    ...
    question_text = st.text_area("❓ Сұрақ", height=130, placeholder="Сұрақты осында жаз...")
    ...
    correct_answer = st.radio("✅ Дұрыс жауап", ["A", "B", "C", "D"], horizontal=True)

    if st.button("💾 Сұрақты сақтау", use_container_width=True):
        answers = [answer_a, answer_b, answer_c, answer_d]

        if question_text.strip() == "" or any(ans.strip() == "" for ans in answers):
            ...
            st.success(f"✅ Сұрақ «{subject}» жалпы базасына сақталды!")
            st.balloons()
...
def combination_page():
    ...
    for subject in common_subjects:
        count = len(questions.get(subject, []))
        if st.button(f"📗 {subject}  •  {count} сұрақ", use_container_width=True, key=f"common_{subject}"):
            st.session_state.selected_subject = subject
            st.session_state.current_question = 0
            st.session_state.user_answers = {}
            st.session_state.active_questions = []
...
# =========================================================
# TEST (АРХИТЕКТУРАЛЫҚ ӨЗГЕРІС: СҰРАҚ ТА, ЖАУАП ТА АРАЛАСАДЫ)
# =========================================================
...
def test_page():
    ...
    # Тек 0-ші сұрақта тұрғанда немесе тізім бос болса ғана сұрақтарды кездейсоқ дайындаймыз
    if st.session_state.current_question == 0 and not st.session_state.active_questions:
        prepared = []
        # Бастапқы тізімнің көшірмесін алып, кездейсоқ араластырамыз
    ...
    if st.button("Келесі →", use_container_width=True):
        if answer is None:
            st.warning("Алдымен жауап таңда.")
        else:
